...
async def should_report_token_limit_when_the_budget_cannot_hold_one_token():
    chunker = HierarchicalChunker(chunk_size=6, chunk_overlap=0)

    with pytest.raises(DocumentLimitError) as caught:
        await chunker.split("# Titre\n\n" + "日本語 " * 20)
    assert caught.value.status_code == 413
...
async def should_split_markdown_nesting_headings_deeper_than_six_levels():
    deep = "####### Détail\n\nCorps du détail."

    chunks = await HierarchicalChunker().split(f"# GR20\n\n{deep}")

    root, leaf = chunks
    assert leaf.content == deep
    assert leaf.metadata["header_level"] == 7
    assert leaf.hierarchy.parent_id == root.id
