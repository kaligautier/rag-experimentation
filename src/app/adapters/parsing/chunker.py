...
from app.ports.rag import TextChunkerPort
...
class HierarchicalChunker(TextChunkerPort):
    ...
    def _emit(
        ...
        if children and section.has_body:
            ...
            budget = self.chunk_size - heading_tokens - 1
            if budget < 1:
                raise self._heading_limit_error(heading_tokens)
            ...
            try:
                parts = splitter.split_text(body if prefix else content)
            except ValueError as e:
                # A tiny budget cannot hold a multi-token character.
                raise DocumentLimitError(
                    "Section body cannot be split within the remaining token budget",
                    details={
    ...
    def _heading_limit_error(self, heading_tokens: int) -> DocumentLimitError:
...
def get_chunker() -> HierarchicalChunker:
    """Get or create the chunker configured from settings."""
    global _chunker_instance
    if _chunker_instance is None:
        _chunker_instance = HierarchicalChunker()
